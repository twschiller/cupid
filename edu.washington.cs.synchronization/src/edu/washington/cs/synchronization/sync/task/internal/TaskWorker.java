...
import edu.washington.cs.synchronization.sync.internal.IProjectModificationListener;
import edu.washington.cs.synchronization.sync.internal.IProjectModificationNotifier;
 ...
 * @see ITask
 */
public class TaskWorker extends BlockableMortalThread implements IProjectModificationNotifier
{
    ...
    /** The list of tasks. */
    private final LinkedList <ITask> tasks_;
    /** A separate set of tasks used for efficient lookup. */
    private final HashSet <ITask> tasksCopy_;
    /** List of tasks of secondary importance. */
    private final HashSet <ISaveTask> saveTasks_;
    ...
    /** a list of listeners. */
    private final ArrayList <IProjectModificationListener> listenerList_;
    ...
    public TaskWorker(long breakTime)
    {
        ...
        tasks_ = new LinkedList <ITask>();
        saveTasks_ = new HashSet <ISaveTask>();
        tasksCopy_ = new HashSet <ITask>();
        listenerList_ = new ArrayList <IProjectModificationListener>();
    ...
    private void addSaveTask(ISaveTask task)
    {
        if (task == null)
            return;
    ...
    private void doSaveTasks()
    {
        taskLock_.lock();
        saveTaskLock_.lock();
        for (ITask task: tasks_)
        {
            ISaveTask save = task.doTask();
            if (save != null)
                saveTasks_.add(save);
        }
        tasks_.clear();
     ...
     * The task is added to {@link #tasks_} if it has {@link ITask#LOWEST_PRIORITY}, or added to {@link #tasksCopy_}
     * otherwise. <br>
    ...
    private void addTask(ITask task, boolean internal)
    {
        assert task != null;
        taskLock_.lock();
        if (!internal)
...
//            result = secondaryTasks_.remove();
    ...
    protected void doWork() throws InterruptedException
    {
        ...
        {
            ISaveTask result = current.doTask();
            addSaveTask(result);
        }
     ...
     */
    @Override
    public void addProjectChangeListener(IProjectModificationListener listener)
    {
    ...
    {
        listenerListLock_.lock();
        for (IProjectModificationListener listener: listenerList_)
